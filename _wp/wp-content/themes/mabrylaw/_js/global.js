...
$script.ready('jquery', function () {
  ...
  $(document).ready(function () {
    ...
    $('.menu-item').wrapInner('<div class="inner"></div>');
    ...
    if (!$('body.home').length) {
      ...
      $("#nav a[href='"+$('h1 a').attr('href')+"'],#primary a[href='"+$('h1 a').attr('href')+"']").each(function () {
    ...
    $('#wpcf7-f1-p209-o1 form').validate(); // Contact Form validation /* Request Legal Services Form validation */
    ...
    $('#wpcf7-f2-p215-o1 form').validate(); // Request Legal Services Form validation
  });
  ...
  function goToItem(carousel, itemClicked) {
    ...
    $currentItem.animate({
      opacity: 'show',
      easing: 'swing'
    }, 1500).removeClass("bleach").addClass('carousel-item-visible').queue(function () {

      $lastItem.animate({
        opacity: 'hide',
        easing: 'swing'
      }, 1500).removeClass('carousel-item-visible');
  ...
  $('#carousel ul').each(function (e) {
    ...
    carouseler = window.setInterval(function () {
      ...
      var $lastItem = carousel.find('li.carousel-item-visible'),
        l = carousel.find('li.carousel-item-visible').next().length,
        $currentItem = (l > 0) ? carousel.find('li.carousel-item-visible').next() : carousel.find('li.carousel-item').first();
